...
def get_model_default_params():
    return dict(
            input_size=(62,1,5), patch_size=(1, 1),channels=62, num_classes=138, model_dim=256,emb_dropout=0.3, pool='cls',
            dim_head=64,channel_rate=4, depth=4, heads=2, mlp_dim=256, dropout=0.3, model_structure="sa+ca"
    )


@register_model
def AITST_classifier(pretrained=False, **kwargs):
    config = get_model_default_params()
    config["num_classes"] = kwargs["num_classes"]
    config["depth"] = 4
    config["heads"] = 2
    print("AITST classifier parameters:", config)
    model = AITST(**config)
...
if __name__ == '__main__':
    x = torch.randn((8, 62, 1, 5))
    model = AITST_classifier(num_classes=138)
    out = model(x)
    print(out.shape)
