...
# 设置数据和标签文件的路径
data_folder = 'E:\\1DomainCrossPI\AffectiveEEGPI\SEED\Preprocessed_EEG/'
label_path = 'E:\\1DomainCrossPI\AffectiveEEGPI\SEED/Preprocessed_EEGlabel.mat'
...
for file_path in mat_files:
    ...
    for key in data.keys():
        if re.match(r'\w+_eeg\d+', key):  # 匹配以任意单词字符开始，后接"_eeg"和数字的键
            eeg_data = data[key]
...
# 数据
positive_data = np.empty((len(positive_label), n_channels, (int)(min_total_samples / window_size), window_size))
neutral_data = np.empty((len(neutral_label), n_channels, (int)(min_total_samples / window_size), window_size))
negative_data = np.empty((len(negative_label), n_channels, (int)(min_total_samples / window_size), window_size))
    ...
    for key in data.keys():
        ...
        if re.match(r'\w+_eeg\d+', key):  # 匹配以任意单词字符开始，后接"_eeg"和数字的键
            ...
            if index <= len(labels):  # 避免索引越界
                ...
                reshaped_data = eeg_data.reshape(n_channels, n_windows, window_size)
...
negative_data_train, negative_data_test, negative_label_train, negative_label_test = train_test_split(negative_data,
                                                                                                      ...
                                                                                                      random_state=42)
# 打印数据形状
print('Positive data:', positive_data_train.shape, 'Positive label:', positive_label_train.shape)
# 为每个情绪保存数据
save_data(positive_data_train, 'Positive\\train_data.pkl')
save_data(positive_data_test, 'Positive\\val_data.pkl')
save_data(positive_label_train, 'Positive\\train_label.pkl')
save_data(positive_label_test, 'Positive\\val_label.pkl')
save_data(neutral_data_train, 'Neutral\\train_data.pkl')
save_data(neutral_data_test, 'Neutral\\val_data.pkl')
save_data(neutral_label_train, 'Neutral\\train_label.pkl')
save_data(neutral_label_test, 'Neutral\\val_label.pkl')
save_data(negative_data_train, 'Negative\\train_data.pkl')
save_data(negative_data_test, 'Negative\\val_data.pkl')
save_data(negative_label_train, 'Negative\\train_label.pkl')
save_data(negative_label_test, 'Negative\\val_label.pkl')
