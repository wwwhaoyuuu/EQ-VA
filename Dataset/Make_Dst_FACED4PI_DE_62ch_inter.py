...
def band_filter(data, sfreq, freq_bands):
    ...
    for idx, band in enumerate(freq_bands):
        ...
        data_filtered = np.expand_dims(mne.filter.filter_data(data, sfreq=sfreq, l_freq=low_freq, h_freq=high_freq), 3)
...
def resample_data(data, origin_sfreq, target_sfreq):
    data_resampled = np.empty(
            (data.shape[0], data.shape[1], data.shape[2] // origin_sfreq * target_sfreq, data.shape[3]))
    for i in range(data.shape[0]):
        for j in range(data.shape[1]):
            for k in range(data.shape[3]):
                data_resampled[i, j, :, k] = resample(data[i, j, :, k], data.shape[2] // origin_sfreq * target_sfreq)
...
def slide_window_extract_de(data, de_window_size, step=5, freq=200):
    ...
    de_array = np.zeros((data.shape[0], n_windows, data.shape[1], de_window_size, data.shape[3]))
...
def get_valance_data(data, stimuli_dict, all=False):
    if all:
        select_emo = {'positive': ['Amusement', 'Inspiration', 'Joy', 'Tenderness'], 'neutral': ['Neutral'],
                      'negative': ['Anger', 'Disgust', 'Fear', 'Sadness']}
    else:
        select_emo = {'positive': ['Joy'], 'neutral': ['Neutral'], 'negative': ['Anger']}
    valance_data_dict = {}
    ...
    for valance in select_emo:
        for emo in select_emo[valance]:
            ...
            valance_data_dict[valance] = valance_data_dict.setdefault(valance, []) + [data[idx]]
            valance_train_data_dict[valance] = valance_train_data_dict.setdefault(valance, []) + [data[idx[:-1]]]
            valance_test_data_dict[valance] = valance_test_data_dict.setdefault(valance, []) + [data[idx[-1]]]
...
def interpolate_missing_channels(data, existing_chs, target_chs, channel_positions, n_jobs=-1):
    num_samples, _, num_timepoints = data.shape
    interpolated_data = np.zeros((num_samples, len(target_chs), num_timepoints))
    ...
    for sample in range(num_samples):
        results = Parallel(n_jobs=n_jobs)(delayed(interpolate_for_timepoint)(
...
if __name__ == '__main__':
    ...
    channels_matrix = [[0, 0, 0, 'FP1', 'FPZ', 'FP2', 0, 0, 0],
    ...
    FACED_chs = ['FP1', 'FP2', 'FZ', 'F3', 'F4', 'F7', 'F8', 'FC1',
                 'FC2', 'FC5', 'FC6', 'CZ', 'C3', 'C4', 'T7', 'T8',
                 'CP1', 'CP2', 'CP5', 'CP6', 'PZ', 'P3', 'P4', 'P7',
                 'P8', 'PO3', 'PO4', 'OZ', 'O1', 'O2', ]

    SEED_chs = ['FP1', 'FPZ', 'FP2', 'AF3', 'AF4', 'F7', 'F5', 'F3', 'F1',
    ...
    info_excel_path = '/data/0shared/jinjiarui/run/PI/Dataset/FACED_Processed_data/Stimuli_info.xlsx'
    stimuli_dict = get_stimuli_info(info_excel_path)
    print("stimuli_dict: ", stimuli_dict)
    file_names = [name for name in os.listdir(data_folder) if fnmatch.fnmatch(name, '*.pkl')]
    ...
    for name in file_names:
        ...
        data = interpolate_missing_channels(data, FACED_chs, SEED_chs, channel_positions, n_jobs=32)
        ...
        de_features = slide_window_extract_de(data_resampled, de_window_size=de_window_size,
                                              step=step, freq=target_sfreq)
        valance_data_dict, valance_train_data_dict, valance_test_data_dict = get_valance_data(de_features,
        ...
        positive_resample = np.arange(
                len(valance_train_data_dict['positive'])) if num_each_sub is None else random.sample(
                list(np.arange(len(valance_train_data_dict['positive']), num_each_sub)))

        neutral_resample = np.arange(
                len(valance_train_data_dict['neutral'])) if num_each_sub is None else random.sample(
                list(np.arange(len(valance_train_data_dict['neutral']), num_each_sub)))

        negative_resample = np.arange(
                len(valance_train_data_dict['negative'])) if num_each_sub is None else random.sample(
                list(np.arange(len(valance_train_data_dict['negative']), num_each_sub)))

        valance_train_data_dict['positive'] = valance_train_data_dict['positive'][positive_resample]
        valance_train_data_dict['neutral'] = valance_train_data_dict['neutral'][neutral_resample]
        valance_train_data_dict['negative'] = valance_train_data_dict['negative'][negative_resample]

        for valance in valance_train_data_dict.keys():
            train_data[valance] = train_data.setdefault(valance, []) + [valance_train_data_dict[valance]]
            train_label[valance] = train_label.setdefault(valance, []) + [label] * len(valance_train_data_dict[valance])
            eval_data[valance] = eval_data.setdefault(valance, []) + [valance_test_data_dict[valance]]
            eval_label[valance] = eval_label.setdefault(valance, []) + [label] * len(valance_test_data_dict[valance])
    ...
    for valance in train_data.keys():
        ...
        print(train_data[valance].shape, train_label[valance].shape, eval_data[valance].shape,
              eval_label[valance].shape)

    os.makedirs(f'/data/0shared/jinjiarui/run/PI/Dataset/FACED_PI/window{de_window_size}_step{step}_ch62',
                exist_ok=True)
    for valance in train_data.keys():
        np.save(f'/data/0shared/jinjiarui/run/PI/Dataset/FACED_PI/window{de_window_size}_step{step}_ch62'
                f'/{valance}_train_data.npy', train_data[valance])
        np.save(
                f'/data/0shared/jinjiarui/run/PI/Dataset/FACED_PI/window{de_window_size}_step{step}_ch62/{valance}_train_label.npy',
                train_label[valance])
        np.save(
                f'/data/0shared/jinjiarui/run/PI/Dataset/FACED_PI/window{de_window_size}_step{step}_ch62/{valance}_eval_data.npy',
                eval_data[valance])
        np.save(
                f'/data/0shared/jinjiarui/run/PI/Dataset/FACED_PI/window{de_window_size}_step{step}_ch62'
                f'/{valance}_eval_label.npy',
                eval_label[valance])

    channel_names = ['FP1', 'FPZ', 'FP2', 'AF3', 'AF4', 'F7', 'F5', 'F3', 'F1',
                     'FZ', 'F2', 'F4', 'F6', 'F8', 'FT7', 'FC5', 'FC3', 'FC1',
                     'FCZ', 'FC2', 'FC4', 'FC6', 'FT8', 'T7', 'C5', 'C3', 'C1',
                     'CZ', 'C2', 'C4', 'C6', 'T8', 'TP7', 'CP5', 'CP3', 'CP1',
                     'CPZ', 'CP2', 'CP4', 'CP6', 'TP8', 'P7', 'P5', 'P3', 'P1',
                     ...
                     'PO4', 'PO6', 'PO8', 'CB1', 'O1', 'OZ', 'O2', 'CB2']
    np.save(
            f'/data/0shared/jinjiarui/run/PI/Dataset/FACED_PI/window{de_window_size}_step{step}_ch62/channel_names.npy',
            channel_names)
