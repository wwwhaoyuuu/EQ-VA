...
import utils
from engine_for_VET import train_one_epoch, evaluate
...
from comparative_model.TransformerEncoder import TF_classifier
import random


def get_args():
    parser = argparse.ArgumentParser("VET Train and Evaluate", add_help=False)
    ...
    parser.add_argument("--tokenizer_model", type=str, default="vqdep_vocab_1k_dim_32")
    parser.add_argument(
        "--codebook_size", default=1024, type=int, help="number of codebook"
    )
    parser.add_argument(
        "--codebook_dim", default=32, type=int, help="dimension of codebook"
    )
...
def get_dataset(args):
    dataset_train = [
        #     [Path("./PI/Dataset/SEED_PI/window1_step1_ch62")],
        [Path("./PI/Dataset/FACED_PI/window1_step1_ch30_sub30")],
    ]
    valance_train = [
        #     [args.emotion],
        [args.emotion]
    ]
    dataset_train_list, train_ch_names_list, num_class = utils.build_dataset(
        dataset_train, valance_train, stage="train"
    )
    ...
    dataset_val_list, val_ch_names_list, num_class = utils.build_dataset(
        dataset_val, valance_val, stage="eval"
    )
...
def main(args):
    ...
    print("number of VAT's params:", n_parameters)
    ...
    if args.eval:
        ...
        print(
            f"Accuracy: {test_stats['accuracy']}, balanced accuracy: {test_stats['balanced_accuracy']}"
        )
        exit(0)
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        if data_loader_val_list is not None:
            val_stats, val_per_dataset = evaluate(
                data_loader_val_list,
                model,
                vqdep,
                device,
            ...
            print(
                f"OA of the network on the {sum([len(dataset) for dataset in dataset_val_list])} "
                f"val EEG: {val_stats['accuracy']:.4f}%"
            )
            print(
                f"BA of the network on the {sum([len(dataset) for dataset in dataset_val_list])} "
            ...
            print(
                f"Max accuracy val: {max_accuracy:.4f}%, epoch: {max_accuracy_epoch} ,dataset1 OA: {dataset1_OA:.4f},dataset2 OA: {dataset2_OA:.4f}"
            )
            ...
            print(
                f"Max balanced accuracy val: {max_balanced_accuracy:.4f}%, epoch: {max_balanced_accuracy_epoch}"
            )
