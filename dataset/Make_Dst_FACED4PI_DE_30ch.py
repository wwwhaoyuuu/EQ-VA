...
if __name__ == "__main__":
    ...
    # num of subjects, a Number or String 'all'
    num_subjects = 30
    ...
    for i, name in enumerate(file_names):
        ...
        if not num_subjects.isdigit():
            num_subjects = len(file_names)
        need = [k for k in range(0, num_subjects)]
        print("The subjects you select: ",need)
    ...
    os.makedirs(
        f"./dataset/FACED_PI/window{de_window_size}_step{step}_ch30_sub{num_subjects}",
        exist_ok=True,
    )
    for valance in train_data.keys():
        np.save(
            f"./dataset/FACED_PI/window{de_window_size}_step{step}_ch30_sub{num_subjects}/{valance}_train_data.npy",
            train_data[valance],
        )
        np.save(
            f"./dataset/FACED_PI/window{de_window_size}_step{step}_ch30_sub{num_subjects}/{valance}_train_label.npy",
            train_label[valance],
        )
        np.save(
            f"./dataset/FACED_PI/window{de_window_size}_step{step}_ch30_sub{num_subjects}/{valance}_eval_data.npy",
            eval_data[valance],
        )
        np.save(
            f"./dataset/FACED_PI/window{de_window_size}_step{step}_ch30_sub{num_subjects}/{valance}_eval_label.npy",
            eval_label[valance],
        )
    ...
    np.save(
        f"./dataset/FACED_PI/window{de_window_size}_step{step}_ch30_sub{num_subjects}/channel_names.npy",
        channel_names,
    )
