...
if __name__ == "__main__":
    ...
    for label, key in enumerate(sub_data_positive.keys()):
        """
            Have 2 method to split dataset into trainset and evalset.
            We can directly take the last experiment as evalset or takes the last trial of each experiment as evalset.
        """
        
        # # directly take the last experiment as evalset
        ...
        # takes the last trial of each experiment
    ...
    # If num_each_sub is None, then use all data of each subject
    positive_resample = (
        np.arange(len(sub_train_positive[next(iter(sub_train_positive))]))
        if num_each_sub is None
        else random.sample(
            list(np.arange(len(sub_train_positive[next(iter(sub_train_positive))]))),
            num_each_sub,
        )
    )
    ...
    neutral_resample = (
        ...
        if num_each_sub is None
            ...
            list(np.arange(len(sub_train_neutral[next(iter(sub_train_neutral))]))),
            num_each_sub,
        )
    )
    ...
    negative_resample = (
        ...
        if num_each_sub is None
            ...
            list(np.arange(len(sub_train_negative[next(iter(sub_train_negative))]))),
            num_each_sub,
        )
    )
    ...
    for label, key in enumerate(sub_train_positive.keys()):
        ...
        print(
            key,
            len(sub_train_positive[key][positive_resample]),
            len(sub_eval_positive[key]),
            len(sub_train_neutral[key][neutral_resample]),
            len(sub_eval_neutral[key]),
            len(sub_train_negative[key][negative_resample]),
            len(sub_eval_negative[key]),
        )

    # merge data of all subjects
    ...
    os.makedirs(
        f"./dataset/SEED_PI/window{de_window_size}_step{step}_ch62", exist_ok=True
    )
    for emo in train_data.keys():
        np.save(
            f"./dataset/SEED_PI/window{de_window_size}_step{step}_ch62/{emo}_train_data.npy",
            train_data[emo],
        )
        np.save(
            f"./dataset/SEED_PI/window{de_window_size}_step{step}_ch62/{emo}_eval_data.npy",
            eval_data[emo],
        )
        np.save(
            f"./dataset/SEED_PI/window{de_window_size}_step{step}_ch62/{emo}_train_label.npy",
            train_label[emo],
        )
        np.save(
            f"./dataset/SEED_PI/window{de_window_size}_step{step}_ch62/{emo}_eval_label.npy",
            eval_label[emo],
        )
        print(
            emo,
    ...
    df = pd.read_excel(
        os.path.join(data_folder_path, "Channel Order.xlsx"), header=None
    )
    channel_names = df.iloc[:, 0].values
    np.save(
        f"./dataset/SEED_PI/window{de_window_size}_step{step}_ch62/channel_names.npy",
        channel_names,
    )
