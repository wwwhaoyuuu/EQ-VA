...
def _load_checkpoint_for_ema(model_ema,
                             checkpoint):
    """
...
def all_gather_batch(tensors):
    ...
    for tensor_all in tensor_list:
        output_tensor.append(torch.cat(tensor_all,
                                       dim=0))
...
def build_dataset(datasets: list,
                  valances: list,
                  stage: str = "train"):
    shock_dataset_list = []
    ch_names_list = []
    start_sub = 0
    for dataset_list, valance_list in zip(datasets,
                                          valances):
        dataset = ShockDataset(dataset_list,
                               valance_list,
                               stage,
                               start_sub)
        start_sub += dataset.get_classes()
        shock_dataset_list.append(dataset)
        ch_names_list.append(dataset.get_ch_names())
    return shock_dataset_list, ch_names_list, start_sub
...
# def get_metrics(output, target, metrics):
