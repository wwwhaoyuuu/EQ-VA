...
def plot_comparison(
    ...
    for i in range(5):  # feature_dim是5
        ...
        ax[i].plot(
            reconstructed_data[:, i].detach().cpu().numpy(), label="Reconstructed_DE"
        )
    ...
    for i in range(5):
        ax[i].plot(
            original_data[:, i].detach().cpu().numpy(), linewidth=5, color="darkblue"
        )
...
def evaluate(
    ...
    for data_loader, ch_names in zip(data_loader_list, ch_names_list):
        ...
        if (epoch + 1) % 1 == 0:
            ...
            original_data_reshaped = original_data.view(
                -1, 5
            )  # 变形为[n_channels * n_times, feature_dim]
            reconstructed_data_reshaped = reconstructed_data.view(
                -1, 5
            )  # 变形为[n_channels * n_times, feature_dim]
            dir_name = args.log_dir.split("/")[-2]
            plot_comparison(
                original_data_reshaped,
                reconstructed_data_reshaped,
                epoch + 1,
        ...
        if epoch == 120:
            ...
            flat_quantize = (
                dataset_quantize.view(-1, dataset_quantize.size(-1)).cpu().numpy()
            )
            ...
            model.module.visualize_tsne(
                flat_data, flat_quantize, flat_labels, epoch, 0, save_path
            )
